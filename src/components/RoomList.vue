        ...
        <section class="rooms" v-cloak>
          <ul class="list-group" id="room-list">
            <li
              ...
              v-bind:class="{ active: room.id == selectedRoom.id }"
              :key="room.id"
...
export default {
  ...
  methods: {
    ...
    selectRoom(index) {
      ...
      this.$http
        .get("/api/v1/rooms/" + this.selectedRoom.id + "/messages")
        .then(response => {
    ...
    addMessage() {
      ...
      this.$http
        .post("/api/v1/rooms/" + this.selectedRoom.id + "/messages", message)
        .then(response => {})
        .catch(error => {
          this.backToHome();
        });
